...
export default function StockPtPage() {
  ...
  useEffect(() => { setPagina(1); }, [grupos.size]);

  const gruposArray    = useMemo(() => Array.from(grupos.entries()), [grupos]);
  const gruposPaginados = useMemo(() =>
    gruposArray.slice((pagina - 1) * POR_PAGINA, pagina * POR_PAGINA),
    [gruposArray, pagina]
  );
  ...
  const diasProm        = lotes.length ? Math.round(lotes.reduce((s, l) => s + diasDesde(l.fecha_produccion), 0) / lotes.length) : 0;
  const lotesAntiguos   = lotes.filter(l => diasDesde(l.fecha_produccion) > 30).length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black uppercase tracking-tight" style={{ color: 'var(--text-main)' }}>Stock de Producto Terminado</h1>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { icon: <Clock size={20} className={lotesAntiguos > 0 ? 'text-red-500' : 'text-amber-500'} />,
                bg: lotesAntiguos > 0 ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200',
                val: `${diasProm}d`, label: lotesAntiguos > 0 ? `${lotesAntiguos} lote(s) >30d` : 'Antigüedad prom.' },
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-muted)' }}>
              <TrendingDown size={12} className="text-slate-400" />Barra = % consumido del lote
            </div>
          </div>
          ...
          {grupos.size === 0 ? (
            ...
            <>
              <div className="space-y-3">
                {gruposPaginados.map(([producto, lotesGrupo]) => {
                  ...
                  return (
                    <div key={producto} className="rounded-xl border-2 border-black/5 overflow-hidden" style={{ background: 'var(--bg-left)' }}>
                      <button onClick={() => toggleExpand(producto)} className="w-full px-5 py-4 flex items-center gap-4 hover:bg-black/3 transition-colors text-left">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {hayAntiguos && <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-red-100 text-red-600">Revisar</span>}
                          </div>
                        ...
                        {resumen && resumen.stock_total > 0 && (
                          <div className="hidden md:flex flex-col items-end gap-1 min-w-[120px]">
                            <p className="text-xs font-black" style={{ color: 'var(--text-main)' }}>{fmtNum(totalGrupo)} <span className="font-normal text-[10px]">{unidad}</span></p>
                            <div className="w-24 h-1.5 rounded-full bg-black/10">
                              <div className="h-1.5 rounded-full bg-blue-500 transition-all" style={{ width: `${Math.min((totalGrupo / resumen.stock_total) * 100, 100)}%` }} />
                            </div>
                          </div>
                        )}
                      ...
                      {isOpen && (
                        <div className="border-t border-black/5">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {lotesGrupo.sort((a, b) => diasDesde(b.fecha_produccion) - diasDesde(a.fecha_produccion)).map(l => {
                                const dias   = diasDesde(l.fecha_produccion);
                                const edad   = edadConfig(dias);
                                const pctCon = l.cantidad_inicial > 0 ? Math.round(((l.cantidad_inicial - l.cantidad_actual) / l.cantidad_inicial) * 100) : 0;
                                return (
                                  <tr key={l.lote_id} className={`border-b border-black/5 border-l-4 ${edad.row} hover:bg-black/2 transition-colors`}>
                                    ...
                                    <td className="px-5 py-3 font-bold" style={{ color: 'var(--text-main)' }}>
                                      {fmtNum(l.cantidad_actual)} <span className="ml-1 text-[10px]" style={{ color: 'var(--text-muted)' }}>{l.unidad_medida}</span>
                                    </td>
                                    ...
                                    <td className="px-5 py-3">
                                      <span className={`px-2 py-0.5 rounded text-[10px] font-black ${edad.badge}`}>{dias}d — {edad.label}</span>
                                    </td>
              ...
              <div className="rounded-xl border-2 border-black/5 overflow-hidden" style={{ background: 'var(--bg-left)' }}>
                <Paginacion total={gruposArray.length} porPagina={POR_PAGINA} pagina={pagina} onChange={setPagina} />
              </div>
            </>
          )}
        </div>
